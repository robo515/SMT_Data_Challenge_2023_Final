...
# To upload every provided file into R, I looped through the files in each folder and created
...
### K-means to predict batter handedness
### This function finds clustering in the mean and variance of the x-location of the starting positions
### across all batters, labeling each unique batter as Righty (R), Switch (S), or Lefty (L).
### Inputs: table_name, a string representing the player_pos table you'd like to analyze
### Outputs: batter_cluster, each batter's unique ID, mean field_x, and predicted handedness (R/S/L)
...
get_batter_cluster_kmeans <- function(table_name) {
  ...
  print(ggplot(data.frame(all_at_bats), aes(x = field_x, y = field_y, color = as.factor(cluster))) +
          ...
          geom_jitter() +
          theme_minimal() +
          labs(title = "Predicted batter handedness 1903", x = "field_x", y= "field_y", color="cluster"))
  # Return the final dataframe
  return(batter_cluster)
}
### Save batter cluster to variable
batter_cluster_kmeans_1903 <- get_batter_cluster_kmeans("player_pos_1903")
batter_cluster_kmeans_1903
value_counts_1903 <- table(batter_cluster_kmeans_1903$cluster)
value_counts_1903
...
# Find all of the BIP with initial bounce in outfield and all BIP initially caught
...
centroids <- centroids[order(centroids$ball_position_x, decreasing = TRUE), ]
...
### The following code block is repeating the outfield Balls In Play K-means analysis for
### Lefties/Righties, this time only in 1903 so we can merge it with the 1903 kmeans results

# To load in game events only from 1903, i created a new folder on my desktop with all the game events from 1903
setwd('/Users/zacambrose/Desktop/game_events1903')
folder <- c('/Users/zacambrose/Desktop/game_events1903')
...
game_events1903 <- dplyr::bind_rows(dataframes)

usable_game_events1903 <- game_events1903 %>%
  group_by(game_str, play_id) %>%
  filter(any(event_code == 4) & any(event_code == 2)) %>%
  select(!X)

# Merge the usable game events (events with a ball in play, ball acquired, or a hit) with game_info
hits_bounces_outfield1903 <- usable_game_events1903 %>%
  inner_join(game_info, by = c("game_str", "play_per_game"))

# Find all of the BIP with initial bounce, and all BIP initially caught
hits_bounces_outfield1903 <- hits_bounces_outfield1903 %>%
  group_by(game_str, play_id) %>%  # for each game and play
  mutate(
    event_shifted = lag(event_code),  # shift event codes down
    bounce_after_bip = event_code == 16 & event_shifted == 4,  # find bounces after ball in play
...
# Balls initially bounced in outfield
bounce_plays1903 <- hits_bounces_outfield1903 %>%
  filter(bounce_after_bip)
# Balls caught in outfield
catch_plays1903 <- hits_bounces_outfield1903 %>%
  filter(acquired_after_bip)

# Now join bounce plays and catch plays with the ball position at the same timestamp, play id, game string as bounce plays and catch plays and 
# find balls hit into outfield
bounce_ball_pos1903 <- inner_join(bounce_plays1903, ball_pos, by = c("game_str", "play_id", "timestamp")) %>%
  mutate(radius = sqrt(ball_position_x**2 + ball_position_y**2)) %>%
  filter(radius > 180)
caught_ball_pos1903 <- inner_join(catch_plays1903, ball_pos, by = c("game_str", "play_id", "timestamp")) %>%
  mutate(radius = sqrt(ball_position_x**2 + ball_position_y**2)) %>%
  filter(radius > 180)
...
# Now we also want to find ALL balls hit into play in outfield in 1903
outfield_BIP1903 <- bind_rows(caught_ball_pos1903, bounce_ball_pos1903)
...
outfield_BIP1903 %>%
  ...
  labs(y = "y-pos", 
       x = "x-pos", 
       title = "Outfield BIP 1903 locations") +
  scale_color_discrete(name = "Player Position")

# Finding all outfield BIP hit by a righty and lefty batters in order to see the difference in
# outfield position clusters
merged_data <- merge(outfield_BIP1903, batter_cluster_kmeans_1903, by = "batter")

# Find all the plays where the batter is a Lefty
lefty_BIP_1903 <- subset(merged_data, cluster == "L")

# Find all the plays where the batter is a Righty
righty_BIP_1903 <- subset(merged_data, cluster == "R")

# Kmeans analysis R vs L:
# Lefties:
ball_positions_1903L <- lefty_BIP_1903[,c('ball_position_x', 'ball_position_y')]
kmeans_result_1903L <- kmeans(ball_positions_1903L, centers=3, nstart=25)
# Create a data frame for the centroids
centroids_1903L <- as.data.frame(kmeans_result_1903L$centers)
names(centroids_1903L) <- c("ball_position_x", "ball_position_y")

lefties_cluster1903 <- ggplot(ball_positions_1903L, aes(x=ball_position_x, y=ball_position_y)) +
  geom_point(aes(color=factor(kmeans_result_1903L$cluster))) + 
  labs(title = 'Outfield BIP clusters, L 1903') +
  scale_color_discrete(name  ="Clusters") +
  theme_minimal()

# Add the centroids to the plot
lefties_cluster1903 + geom_point(data=centroids_1903L, aes(x=ball_position_x, y=ball_position_y), size=6, shape=4, color="black")

# Righties:
ball_positions_1903R <- righty_BIP_1903[,c('ball_position_x', 'ball_position_y')]
kmeans_result_1903R <- kmeans(ball_positions_1903R, centers=3, nstart=25)
# Create a data frame for the centroids
centroids_1903R <- as.data.frame(kmeans_result_1903R$centers)
names(centroids_1903R) <- c("ball_position_x", "ball_position_y")

# Create the base plot
righties_cluster1903 <- ggplot(ball_positions_1903R, aes(x=ball_position_x, y=ball_position_y)) +
  geom_point(aes(color=factor(kmeans_result_1903R$cluster))) + 
  labs(title = 'Outfield BIP clusters, R 1903') +
  scale_color_discrete(name  ="Clusters") +
  theme_minimal()

# Add the centroids to the plot
righties_cluster1903 + geom_point(data=centroids_1903R, aes(x=ball_position_x, y=ball_position_y), size=6, shape=4, color="black")

print(centroids_1903R)
print(centroids_1903L)

setwd('/Users/zacambrose/Downloads/SMT_Data_2023')

# Outfield parameters:
# (0, 60) is the mound, and we defined a radius of 100 feet outside this
...
# The "outfield_plays" dataframe now contains all the plays where a flyball was hit outside the outfield circle with center (0, 60) and radius 100 feet.


#Find all game events where player_pos = 7, 8 or 9, & where event code = 2
...
# Used in hits_bounces_outfield to find all balls hit into play and received
usable_game_events1903 <- game_events1903 %>%
  group_by(game_str, play_id) %>%
  filter(any(event_code == 4) & any(event_code == 2)) %>%
  select(!X)
...
# Hits + bounces in outfield, 1903
hits_bounces_outfield1903 <- game_events1903 %>%
  group_by(game_str, play_id) %>%  # for each game and play
  mutate(
...
ballpos_bounces_hits <- merge(hits_bounces_outfield1903, ball_pos, by = c("game_str", "play_id", "timestamp")) %>%
  mutate(ball_dist_center = sqrt((ball_position_x - outfield_center_x)^2 + (ball_position_y - outfield_center_y)^2))

# First outfield bounces
...
# Relevant columns from outfield bounces
...
# Merge hit_game_events with player_pos1903
hit_time_when_ball_bounced1903 <- left_join(hit_game_events, player_pos_1903, by=c("game_str","play_id","timestamp"))
hit_time_when_ball_bounced1903 <- hit_time_when_ball_bounced1903[hit_time_when_ball_bounced1903$player_position.y == 7 | hit_time_when_ball_bounced1903$player_position.y == 8 |hit_time_when_ball_bounced1903$player_position.y == 9,]
outfield_hit_time_when_ball_bounced1903 <- subset(hit_time_when_ball_bounced1903, substr(game_str,1,4) == "1903")
# outfield_hit_time_when_ball_bounced1903 is the game, play id where the ball bounced with the timestamp when the ball was hit and the player positions when the ball was hit
...
# acquired_after_bounce1903 is the game events where the ball was acquired after the ball was bounced
acquired_after_bounce1903 <- game_events1903 %>%
  group_by(game_str, play_id) %>%  # for each game and play
  mutate(
    event_last = lag(event_code), # shift event codes down
...
acquired_after_bounce1903 <- acquired_after_bounce1903 %>%
...
# Subsetting acquired after bounce to find hits and bounces
bounces <- acquired_after_bounce1903[acquired_after_bounce1903$event_code == 16,] %>%
  group_by(game_str, play_id)
bounces <- rename(bounces, timestamp_bounce = timestamp)
hits <- acquired_after_bounce1903[acquired_after_bounce1903$event_code == 4,] %>%
  group_by(game_str, play_id)
...
# filtered_events = game events where an outfielder acquired a ball
filtered_events <- filtered_events[kept]

# newdf is all the hits and bounces with timestamps joined with filtered_events 
# to make a comprehensive dataframe
newdf <- hits_and_bounces %>%
  left_join(filtered_events, by = c("game_str","play_id")) %>%
  drop_na('player_position.y')

# bounce_ball_df is a comprehensive dataframe with all relevant info for determining
# catchable balls that bounced
bounce_ball_df <- newdf %>%
  left_join(bounce_ball_pos_cleaned, by = c("game_str","play_id")) %>%
  drop_na('bounce_position_x')


bounce_ball_df <- left_join(bounce_ball_df, player_pos_1903, by = c("game_str" = "game_str", "play_id" = "play_id", "player_position.y" = "player_position", "timestamp_hit" = "timestamp"))
...
# putting timestamps into seconds, calculating hangtime
...
player_1903_outfield_pos_rf <- player_pos_1903[player_pos_1903$player_position == 9,]
player_1903_outfield_pos_cf <- player_pos_1903[player_pos_1903$player_position == 8,]
player_1903_outfield_pos_lf <- player_pos_1903[player_pos_1903$player_position == 7,]

player_1903_outfield_pos <- rbind(player_1903_outfield_pos_rf, player_1903_outfield_pos_cf, player_1903_outfield_pos_lf)

### Now, find average starting positions for each position, LF, CF, RF
rf_starting_positions <- player_1903_outfield_pos_rf %>%
  group_by(game_str, play_id) %>%
  filter(timestamp == min(timestamp)) %>%
  ungroup()

cf_starting_positions <- player_1903_outfield_pos_cf %>%
  group_by(game_str, play_id) %>%
  filter(timestamp == min(timestamp)) %>%
  ungroup()

lf_starting_positions <- player_1903_outfield_pos_lf %>%
  group_by(game_str, play_id) %>%
  filter(timestamp == min(timestamp)) %>%
  ungroup()

# Cf starting pos means
cf_avg_pos <- c(mean(cf_starting_positions$field_x),mean(cf_starting_positions$field_y))

# Rf starting pos means
rf_avg_pos <- c(mean(rf_starting_positions$field_x),mean(rf_starting_positions$field_y))


# Lf starting pos means
...
plotlf <- ggplot(lf_bounces, aes(x=lf_bounces$bounce_position_x,y=lf_bounces$bounce_position_y)) +
  geom_point()
...
# running_splits.csv Source: https://baseballsavant.mlb.com/running_splits?type=raw&bats=&year=2023&position=&team=&min=5
...
### Calculating outs with kmeans positions:
centroids
rf_kmeans_pos <- c(centroids[1,])
cf_kmeans_pos <- c(centroids[2,])
lf_kmeans_pos <- c(centroids[3,])

# Here, we plot the average starting position of outfielders at the time of the ball
# being hit in purple, the optimized kmeans position in gold, 
...
rf_bounces$out <- ifelse(20.314 * rf_bounces$hangtime > rf_bounces$bounce_dist_from_kmeanstart, 1, 0)
rf_kmeans_pos
...
lf_bounces$out <- ifelse(20.314 * lf_bounces$hangtime > lf_bounces$bounce_dist_from_kmeanstart, 1, 0)
...
cf_bounces$out <- ifelse(20.314 * cf_bounces$hangtime > cf_bounces$bounce_dist_from_kmeanstart, 1, 0)
...
ball_hit_in_play_test <- merge(player_1903_outfield_pos, ball_hit_in_play, by = c("game_str", "play_id", "timestamp"))

#This is all of the timestamps for when ball was acquired along with positions.
ranges_1903 <- inner_join(filtered_events, player_1903_outfield_pos, by = c("game_str", "play_id", "player_position", "timestamp"))

#ball caught
ranges_1903 <- ranges_1903 %>%
...
official_outfield_range_data <- merge(ranges_1903, ball_hit_in_play_real, by = c("game_str", "play_id", "player_position"))
...
right_fielder_ranges$out <- ifelse(20.314 * right_fielder_ranges$time_difference_sec > right_fielder_ranges$bounce_dist_from_kmeanstart, 1, 0)
...
center_fielder_ranges$out <- ifelse(20.314 * center_fielder_ranges$time_difference_sec > center_fielder_ranges$bounce_dist_from_kmeanstart, 1, 0)
...
left_fielder_ranges$out <- ifelse(20.314 * left_fielder_ranges$time_difference_sec > left_fielder_ranges$bounce_dist_from_kmeanstart, 1, 0)
...
#Creating a dataset manually becaise it was easier
Pos_year <- c("lf_1902", "cf_1902", "rf_1902", "lf_1903", "cf_1903", "rf_1903")
outs_generated <- c(30, 16, 21, 26, 25, 25)
outs_lost <- c(15, 17, 14, 15, 21, 25)

overall_results <- data.frame(Pos_year, outs_generated, outs_lost)
overall_results
overall_results$total <- overall_results$outs_generated - overall_results$outs_lost

#Plot that shows the amount of outs generated - outs lost to show that our starting location
#is net positive
p <- ggplot(overall_results, aes(x = Pos_year, y = total, fill = colors)) +
  geom_bar(stat = "identity") +
  labs(title = "Outs Generated Minus Outs Lost", x = "Position & Year", y = "Outs") +
  scale_fill_manual(values = c("red" = "red", "green" = "green3")) +
  theme_minimal()

# Add labels with adjusted size
p + geom_text(aes(label = total), vjust = -0.5, size = 4)
